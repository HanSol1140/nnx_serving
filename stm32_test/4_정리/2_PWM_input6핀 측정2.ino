...
void setupTimer(HardwareTimer *timer, int pwmPin, uint32_t channelRising, uint32_t channelFalling) {
  ...
  // 전역 변수로 채널 번호 업데이트
  if (pwmPin == pwmPin1) {
    channelRising1 = channelRising;
    channelFalling1 = channelFalling;
  } else if (pwmPin == pwmPin2) {
    channelRising2 = channelRising;
    channelFalling2 = channelFalling;
  } else if (pwmPin == pwmPin3) {
    channelRising3 = channelRising;
    channelFalling3 = channelFalling;
  } else if (pwmPin == pwmPin4) {
    channelRising4 = channelRising;
    channelFalling4 = channelFalling;
  } else if (pwmPin == pwmPin5) {
    channelRising5 = channelRising;
    channelFalling5 = channelFalling;
  } else if (pwmPin == pwmPin6) {
    channelRising6 = channelRising;
    channelFalling6 = channelFalling;
  }
}
...
void setup() {
  ...
  setupTimer(timer1, pwmPin1, 1, 2);  // TIM1 - 1채널, 2채널
  setupTimer(timer1, pwmPin2, 3, 4);  // TIM1 - 3채널, 4채널
  setupTimer(timer2, pwmPin3, 1, 2);  // TIM2 - 1채널, 2채널
  setupTimer(timer2, pwmPin4, 3, 4);  // TIM2 - 3채널, 4채널
  setupTimer(timer3, pwmPin5, 1, 2);  // TIM3 - 1채널, 2채널
  setupTimer(timer3, pwmPin6, 3, 4);  // TIM3 - 3채널, 4채널
